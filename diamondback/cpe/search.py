...
def _load_config(config_file=DEFAULT_CONFIG_FILE, path_to_db=DEFAULT_PATH_TO_DATABASE ):
    ...
    with open(config_file) as f:  # default: config.json
        ...
        if path_to_db != DEFAULT_PATH_TO_DATABASE:
            logger.info( 'changing path to CPE database' )
            config['DATABASE_NAME'] = path_to_db
            with open( config_file, 'w' ) as writer:
                logger.info( 'saving updated JSON to disk' )
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                if cpe_api_data_response.status == 200:
                    if DEBUG:
                        logger.info( f"[+] Successfully received data from request {requestno}." )
                    return await cpe_api_data_response.json()
                else:
                    if DEBUG:
                        logger.info( f"[-] Received status code {cpe_api_data_response.status} on request {requestno} Retrying..." )
                await asyncio.sleep(retry_interval)
            except Exception as e:
                if UPDATE_SUCCESS and not SILENT:
                    logger.info( 'Got the following exception when downloading CPE data via API: %s' % str(e) )
...
def intermediate_process(api_data, requestno):
    ...
    if DEBUG:
        logger.info(f"[+] Intermediate processing on request number {requestno}")
...
async def worker( headers, params, requestno, rate_limit ):
    ...
    if api_data_response is not None:
        try:
            ...
            if DEBUG:
                logger.info( f"[+] Performing calculations on request number {requestno}" )
        ...
        UPDATE_SUCCESS = False
        if not SILENT:
            logger.info('api_data_response appears to be None.')
        return None
...
async def update( nvd_api_key=None, config=None ):
    ...
    if not SILENT:
        logger.info( "[+] Getting NVD's official CPE data (might take some time)" )
    ...
    if nvd_api_key:
        ...
        if not SILENT:
            logger.info( '[-] No API Key found - Requests will be sent at a rate of 5 per 30s. To lower build time, consider getting an NVD API Key.' )
    ...
    if numTotalResults == -1:
        logger.info( 'Got the following exception when getting CPE count data via API: %s' % str(exception) )
        return False
    ...
    if db_type == 'mariadb':
        if not is_safe_database_name(db_name):
            logger.info( 'Potential malicious database name detected. Abort creation of database' )
            return False
    ...
    with open(config['DEPRECATED_CPES_FILE'], "w") as outfile:
        ...
        outfile.close()
    logger.info( 'async updated complete, moving on' )
    return True
...
def update_local_database( api_key=None, config=None ):
    logger.info( 'start update of CPE database' )
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete( update( api_key, config) )

    if not UPDATE_SUCCESS:
        logger.info('[-] Failed updating the local CPE database!')
...
def search_cpes( query, count=3, threshold=-1, config=None ):
    ...
    if config['DATABASE']['TYPE'] == 'sqlite' and not os.path.isfile( config['DATABASE_NAME'] ):
        if not SILENT:
            logger.info( "[+] Running initial setup (might take a couple of minutes)" )
        update_local_database( config=config )
        logger.info( 'update complete, move onto query' )

    query          = query.strip()
    cpes, pot_cpes = [], []

    if not MATCH_CPE_23_RE.match( query ):
        cpes = _search_cpes(    [query], 
                                count     = count, 
                                threshold = CPE_SEARCH_THRESHOLD_ALT, 
                                config    = config )
...
if __name__ == "__main__":
    ...
    if args.queries:
        ...
        for i, query in enumerate(results):
            ...
            if results[query]:
                logger.info(results[query][0][0])
                if not SILENT:
                    pprint.pprint(results[query])
            else:
                logger.info( 'Could not find software for query: %s' % query )
                if not SILENT:
                    pprint.pprint([])
